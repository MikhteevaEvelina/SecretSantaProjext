...
from createBot import dp, bot, webhook_url, text_rules
...
async def santa_password(message : types.Message, state: FSMContext):
    ...
    async with state.proxy() as data:
        ...
        token = get_token(r, 'token')
        id = get_token(r, 'id')
        r = requests.get(webhook_url + '/api/pair/' + str(id), headers={'Content-Type': 'application/json', 'Authorization' : token})
        if r.status_code == 200:
            rec_user = dict(r.json())
            rec_user = dict(rec_user['receiverUser'])
            await bot.send_message(message.chat.id, 'имя: ' + str(rec_user['name']) + ', пол: ' + str(rec_user['sex']) + ', возраст: ' + str(rec_user['age']) + ', почтовый адрес :' + str(rec_user['post_adr']) + ', хобби: ' + str(rec_user['userHobby']))
        else:
...
async def games_rules(message : types.Message):
    await bot.send_message(message.chat.id, text_rules)
...
def register_handlers_client(dp : Dispatcher):
    ...
    dp.register_message_handler(cm_santa, commands = ['Для_кого_я_Санта'], state=None)
